...
- **A payment argument with its own signer is two wallet popups, not one group.** `AtomicTransactionComposer.simulate()` calls `gatherSignatures()`, which batches by function identity. Resource discovery used the empty simulate signer on the app call and left the live wallet on the payment; Pera asked for the payment, then asked again for the group. The builders now pass the composer signer into every payment and axfer argument, and discovery encodes the group as unsigned simulate blobs so it never prompts. `js/test/rain-abi.test.ts` pins both halves.
